...
export async function POST(req: NextRequest) {
  ...
  const { email, password, full_name, age, gender, blood_group, phone, address } = body;
...
if (password.length < 8 || !hasSpecialChar || !hasNumber) {
  ...
  try {
    ...
    await db.query(
      `INSERT INTO users (id, email, password_hash, full_name, age, gender, blood_group, phone, address)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, email, password_hash, full_name, age, gender, blood_group || null, phone || null, address || null]
    );
